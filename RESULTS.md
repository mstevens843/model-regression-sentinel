...
| tests | **267 green**, 13 tasks: spec 75, report 50, watch 37, baseline 36, cli 25, run 22, detect 22 |
| the frozen corpus validates and matches its manifest | pass, 24 cases |
| `shasum -a 256 -c` agrees, with no code from this project | pass, 6 files |
| the freeze ordering proof | **exits 1 by design.** UNAVAILABLE, not pending |
| `pnpm audit:release`, including two negative controls that must fire | all green |
| calibration scenarios against the reference detector | 11 of 11 |
| detector mutants caught | **9 of 9, zero escapes** |
...
## Mutant discrimination: 9 of 9

`pnpm -F @model-regression-sentinel/detect test`

| mutant | declared `mustFail` | actually failed | escapes |
|---|---|---|---|
| `rawDiff` | 01, 03, 04 | 01, 03, 04, 07, 09, 10 | **0** |
| `alwaysQuiet` | 02, 06, 09 | **02, 06, 09** | **0** |
| `alwaysDrift` | 01, 03, 04, 07, 08 | 01, 03, 04, 05, 07, 08, 09, 10 | **0** |
| `meanLatencyGate` | 04 | 01, 04 | **0** |
| `peeks` | 05 | **05** | **0** |
| `noConfirmation` | 09 | **09** | **0** |
| `anyCorpus` | 08 | **08** | **0** |
| `singleReplicateOk` | 07 | **07** | **0** |
| `hidesDebt` | 11 | 05, 11 | **0** |

Four of these are caught by **exactly one scenario each**, and that is the number to watch across
releases. Adding scenarios is the easy way to make an existing mutant fail more broadly, and a mutant
whose blast radius quietly widens means the new scenarios are blunt rather than sharp.
...
latency, never manufactures an alarm from repeated looks. A test asserts that running only those six
...
| **The corpus ordering proof** | **UNAVAILABLE, not pending.** No git operation was permitted in the environment that produced this repository, so the corpus could not be committed before the detector existed. `pnpm verify:freeze` exits 1 by design and `corpus/*/FREEZE.json` carries the four commands a human would run. |
| **`NO_DRIFT` is unreachable on this corpus** | Not one of 200 A/A splits returned it. `schemaValid` exists on only two cases, which gives the sign-flip test four possible assignments and a smallest attainable p of 0.25, so no effect of any size can reach significance on it. The suite answers `INCONCLUSIVE` instead. Honest, and inconvenient. |
