...
| **The corpus ordering proof** | **PERMANENTLY UNAVAILABLE, not pending.** The proof requires a commit at which `packages/detect/src/compare.ts` is absent; it exists in the first commit, so no commit that will ever exist can satisfy it. `pnpm verify:freeze` exits 1 by design. `pnpm verify:precedence` cashes a weaker, separately named claim - each split was committed no later than every run measured against it - and since `results/runs-v2/` was committed after the schema split, **all three splits now pass it in the strict form**. That is a different sentence from the freeze proof and must not be quoted as one. |
...
| **Every published claim rests on one provider** | The `codex_cli` adapter has since made 40 live plan-backed calls against a second vendor, so the seam is no longer demonstrated against one CLI only. That is **supplementary** and gates nothing: every number in `results/runs/`, `results/runs-v2/`, both calibration studies and every generated block still comes from one CLI on one laptop against one vendor. Nothing establishes that the noise floors, the error rates or the alias-resolution behaviour resemble any other provider's - and Codex in particular discloses no model identity at all, so it cannot corroborate the alias-resolution wedge. |
